...
from energy_utils.models import EnergyDayAhead
...
class Prices:
    ...
    def next24hours(self, price_zone="SE_4"):
        ...
        df = pd.DataFrame(self.client.query_day_ahead_prices(price_zone, start=start, end=end), columns=["prices"])
        for row in df.itertuples():
            EnergyDayAhead.objects.update_or_create(price=float(row[1]), time=row[0], energy_zone=price_zone)
        pprint(df.to_string())
